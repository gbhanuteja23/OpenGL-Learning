...
void Model::Draw(Shader& shader, Camera& camera)
{
	...
	{
		// Create a rotation matrix for 180 degrees around the Y-axis
		glm::mat4 rotationFix = glm::rotate(glm::mat4(1.0f), glm::radians(180.0f), glm::vec3(1.0f, 0.0f, 0.0f));

		// Apply this fix to the mesh's original matrix
		glm::mat4 modelMatrix = GetTransform() * rotationFix * matricesMeshes[i];

		// Pass the corrected matrix to the drawing function
...
glm::mat4 Model::GetTransform() const
{
	...
	glm::mat4 rot = rotZ * rotY * rotX; // ZYX rotation order
...
)
{
	...
	{
		// Flip the V (or Y) component of the texture coordinate
		glm::vec2 correctedTexUV = glm::vec2(texUVs[i].x, 1.0f - texUVs[i].y);
